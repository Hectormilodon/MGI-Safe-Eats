...
const generatePassword = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let index = 0; index < cantidad; index++) {
        ...
        const newHash = (0, bcrypt_1.generateHash)(pass);
...
const post_user = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const data = req.body;
        // if (!data.rut || data.rut.trim().length === 0) {
        // 	data.rut = 0;
        // }
        // const pass = Math.random().toString(36).slice(-8);
        // data.password = pass;
        ...
        if (userCreated.err) {
            ...
            // let body = { user: data.email };
            /*
            await reportService.createUserInternal(body);
            if (sendEmail) {
                const sended = await sendMailCreate(userBuilded.email, pass);
            }*/
        }
        /*if ("local" in data) {
            (data.local as any);
            ...
            if (usercompanies.length > 0)
                await userCompany.bulkCreate(usercompanies).catch((reason) => {
        ...
        if ("rol" in data) {
            ...
            await labelUser.create(userlabel).catch((reason) => {
                console.log("🚀 ~ reason:", reason);
            });
        }*/
